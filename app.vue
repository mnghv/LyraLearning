...
useHead({
  title: 'Mnghv | Lyra Learning',
})
</script>

<template>
  <div class="min-h-screen bg-slate-100">
    <header class="flex items-center gap-3 p-2 justify-between sticky top-0 z-10 bg-slate-50 border-b border-slate-200">
      <NuxtLink to="/" class="text-purple-600 flex gap-1 items-center pl-2">
        ...
        <span class="font-bold text-purple-800 inline-block">Mnghv</span>
        <span class="hidden sm:inline">|</span>
        <span class="hidden sm:inline"> Lyra Learning</span>
      </NuxtLink>
      <div class="flex items-center gap-8">
        ...
        <nav class="flex items-center justify-end gap-2 w-48">
          <Transition enter-active-class="duration-200 ease-in" enter-from-class="transform opacity-0"
            enter-to-class="opacity-100">
            <div v-if="!pending && user">
              <ActionButton @click="login" size="small" v-if="!user || !('githubId' in user)">
                Login with GitHub
              </ActionButton>
              <ActionButton @click="logout" size="small" v-else>
                <img :src="user.avatarUrl" class="w-5 rounded-full border border-slate-300 mr-2" />
                Logout
              </ActionButton>
            </div>
          </Transition>
          <a href="https://github.com/mnghv" class="flex px-2 py-1 text-slate-600 hover:text-slate-800"
            title="Open GitHub Repo" target="_blank">
            <mdicon name="github" />
          </a>
        </nav>
      </div>
    ...
    <footer class="p-8 flex justify-center items-center gap-1 text-slate-500">
      Built by
      <a href="https://www.linkedin.com/in/mnghv/" target="_blank" class="inline-flex items-center">
        <mdicon name="linkedin" size="20" /> Mnghv
      </a>
    </footer>
  </div>
</template>
